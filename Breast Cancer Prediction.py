...
# Set a threshold value for identifying outliers using z-scores. Iterate through each column in the DataFrame and calculate z-scores for each data point. Mark data points as outliers if their absolute z-score exceeds the predefined threshold.
# 
# Checking the number of outliers. All outliers have been successfully removed from the dataset by Interquartile Range method 
...
# Print indices of outliers for each column
for i, column in enumerate(df.columns):
    z_scores = stats.zscore(df[column])
    outliers = (abs(z_scores) > threshold)
    outlier_indices = df.index[outliers].tolist()

    print(f"Column: {column}, Number of Outliers: {sum(outliers)}, Indices: {outlier_indices}")
